 ...
 *
 * wordA is a predecessor of wordB if and only if we can insert exactly one letter anywhere in wordA without changing the order of the other
...
public class LongestStringChain {
    public int longestStrChain(String[] words) {
        ...
        for(String word : words) {
            ...
            int subMax = map.get(word);
            for(int j= 0; j<word.length(); j++) {
                StringBuilder temp = new StringBuilder(word);
                String prev = temp.deleteCharAt(j).toString();
                if(map.containsKey(prev)) {
                    subMax = Math.max(subMax, map.get(prev) +1);
                }
            }
            map.put(word, subMax);
